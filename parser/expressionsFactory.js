...
function variableDeclaration(tokens, start) {
  ...
  return {
    type: constParser.expressionDeclaration,
...
function variableAffectation(tokens, start) {
  ...
  return {
    type: constParser.expressionAffectation,
    variableName: variableName,
...
function functionDeclaration(tokens, start) {
  ...
  if (tokens[start + 2].type != constTokens.symboleOpenParenthese)
  ...
  if (tokens[startAndArg + 3].type != constTokens.symboleCloseParenthese)
    throw constParser.errorMissingCloseParenthesis;

  if (tokens[startAndArg + 4].type == constTokens.typeTypeHint) {
    if (tokens[startAndArg + 5].type != constTokens.typeWord)
      throw constParser.errorMissingWord;

    functionTypeHint = tokens[startAndArg + 5].value;

    if (tokens[startAndArg + 6].type != constTokens.symboleColon)
      throw constParser.errorMissingColon;

    tokenEnd = startAndArg + 6;
  } else {
    if (tokens[startAndArg + 4].type != constTokens.symboleColon)
      throw constParser.errorMissingColon;
  ...
  return {
    type: constParser.expressionFunctionDeclaration,
    functionName: functionName,
    functionTypeInt: functionTypeHint,
    functionArgs: arguments.args,
    start: start,
    end: tokenEnd,
  };
}
...
function expressionIndent(tokens, start) {
  ...
  return {
    type: constParser.expressionIndent,
    quantity: i,
    start: start,
    end: start + i - 1,
  };
